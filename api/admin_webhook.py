...
@admin_webhook_router.post("/admin/webhook")
def admin_webhook(
    ...
    expected = admin_settings.webhook_secret.get_secret_value()
    if not expected or not admin_settings.token.get_secret_value():
        raise HTTPException(status_code=503, detail="Admin webhook is not configured")
    if not telegram_secret_token or not secrets.compare_digest(
        telegram_secret_token, expected
    ):
