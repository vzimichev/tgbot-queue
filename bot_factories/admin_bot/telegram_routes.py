...
from bot_factories.admin_bot.repository import (
    BotRepository,
    Repository,
    TelegramAPI,
    cache_folder,
...
class AdminService:
    def __init__(self, api, repository, owner_id):
        ...
        self.owner = owner_id
        self.bots = BotRepository(api, repository, owner_id)
    ...
    def handle(self, update):
        ...
        if message.get("users_shared"):
            ...
            if pending:
                ...
                self.say("Создание бота уже начато. Сколько секунд добавить к лимиту?")
                return
        ...
        if command in ("/start", "/help"):
        ...
        elif command == "/create" or text in (
            "Создать бота",
            "Создать бота / добавить лимит",
        ):
            ...
            if draft["step"] == "recipient":
                ...
                if draft["step"] == "add_limit":
                    ...
                    increment = int(text)
                    try:
                        record = self.bots.add_limit(key, increment)
                    except LookupError:
                        self.repo.delete("draft")
                ...
                if self.recipient_taken(draft["recipient_id"]):
                    self.repo.delete("draft")
                    return
                pending = self.bots.create_pending(draft, int(text))
    ...
    def register(self, bot):
        record = self.bots.register(bot)
        if record is None:
            self.say(
                f"Получен бот @{bot.get('username', '')}, но его параметры не найдены. "
                "При создании нужно сохранить предложенный @username."
            )
            return
        self.say("Бот сохранён.", reply_markup=self.main_keyboard())
        self.say(self.describe(record), reply_markup=self.share_keyboard(record))

    def choose_recipient(self):
        request_id = secrets.randbelow(2**31)
        self.repo.put("draft", {"step": "recipient", "request_id": request_id})
        self.say(
    ...
    def claim_bot(self, message, claim_token):
        record = self.bots.find_claim(claim_token)
        sender = message.get("from", {})
        sender_id = sender.get("id")
        result = self.bots.activate_claim(record, sender, claim_token)
        if result == "invalid":
            self.api.call(
                "sendMessage",
                chat_id=sender_id,
